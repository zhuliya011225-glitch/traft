  ...
  return (
    ...
    >
      {rows.map((_, i) => (
        <motion.div
          key={`row` + i}
          className="w-16 h-8  border-l  border-slate-700/5 relative"
        >
          {cols.map((_, j) => (
            <motion.div
              whileHover={{
              ...
              key={`col` + j}
              className="w-16 h-8  border-r border-t border-slate-700/5 relative"
            >
              {j % 2 === 0 && i % 2 === 0 ? (
                <svg
...
export const BoxesCore = React.memo(Boxes);
